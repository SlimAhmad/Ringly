...
public partial class AsteriskCallCenterFoundationServiceTests
{
    ...
    [InlineData("")]
    [InlineData(" ")]
    public async Task ShouldThrowValidationExceptionOnPauseRecordingIfRecordingNameIsInvalidAndLogItAsync(
        string? invalidRecordingName)
    {
        ...
        // when
        ValueTask pauseTask =
            this.asteriskCallCenterFoundationService.PauseRecordingAsync(invalidRecordingName!);

        RecordingValidationException actualException =
            await Assert.ThrowsAsync<RecordingValidationException>(pauseTask.AsTask);
