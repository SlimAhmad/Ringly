...
public partial class AsteriskCallCenterFoundationServiceTests
{
    ...
    [InlineData("")]
    [InlineData(" ")]
    public async Task ShouldThrowValidationExceptionOnUnpauseRecordingIfRecordingNameIsInvalidAndLogItAsync(
        string? invalidRecordingName)
    {
        ...
        // when
        ValueTask unpauseTask =
            this.asteriskCallCenterFoundationService.UnpauseRecordingAsync(invalidRecordingName!);

        RecordingValidationException actualException =
            await Assert.ThrowsAsync<RecordingValidationException>(unpauseTask.AsTask);
