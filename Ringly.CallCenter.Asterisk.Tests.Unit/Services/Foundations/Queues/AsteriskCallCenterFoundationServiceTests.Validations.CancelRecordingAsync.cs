...
public partial class AsteriskCallCenterFoundationServiceTests
{
    ...
    [InlineData("")]
    [InlineData(" ")]
    public async Task ShouldThrowValidationExceptionOnCancelRecordingIfRecordingNameIsInvalidAndLogItAsync(
        string? invalidRecordingName)
    {
        ...
        // when
        ValueTask cancelTask =
            this.asteriskCallCenterFoundationService.CancelRecordingAsync(invalidRecordingName!);

        RecordingValidationException actualException =
            await Assert.ThrowsAsync<RecordingValidationException>(cancelTask.AsTask);
